...
const SupplierPayments = () => {
  ...
  const fetchPayments = async () => {
    try {
      setLoading(true);
      const response = await supplierPaymentsService.getAll();
      ...
      setError('Failed to load supplier payments. Please try again.');
      setPayments([]);
    } finally {
      setLoading(false);
    }
  ...
  const calculateSummary = (paymentsData) => {
    ...
    setSummary({
      ...
      outstanding: 0, // Calculate from bookings if needed
    });
  };
  ...
  const filteredPayments = payments.filter((payment) => {
    ...
    const matchesPaymentTo = paymentToFilter === '' || payment.payment_to === paymentToFilter;
    const matchesMethod = paymentMethodFilter === '' || payment.payment_method === paymentMethodFilter;

    return matchesSearch && matchesPaymentTo && matchesMethod;
  });
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-600 mt-1">Track payments made to suppliers</p>
          </div>
          ...
          <Card>
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{formatCurrency(summary.total_paid)}</p>
              </div>
            </div>
          </Card>
          ...
          <Card>
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{formatCurrency(summary.outstanding)}</p>
              </div>
            </div>
          </Card>
        </div>
        ...
        <Card>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by booking, supplier, reference..."
                value={searchTerm}
            ...
            >
              ...
              <option value="tour_supplier">Tour Supplier</option>
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {paginatedPayments.map((payment) => (
                      <tr key={payment.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 text-sm text-slate-600">{payment.supplier_name}</td>
                        <td className="px-4 py-3 text-sm">
                          <Badge variant="info">
                            {payment.payment_to?.replace('_', ' ').toUpperCase()}
                          </Badge>
                        </td>
        ...
        <Modal
          ...
          title={editingPayment ? 'Edit Supplier Payment' : 'Add New Supplier Payment'}
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <Input
                ...
                placeholder="Hotel name, tour operator, guide name, etc."
                required
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('payment_to', e.target.value)}
